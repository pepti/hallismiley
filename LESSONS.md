...
## 2026-09-12 — the e2e runner was not flaky, it was racing the image's own apt-get (factory)
...
### 2026-09-24 — an engine test that asserts the ENGINE default must build it

_(factory)_ Two engine tests said "the default" but read the product's contract: `moduleFlags.test.js` ("every module, as before R4") and `featureGate.test.js` ("the public-site specs skip; everything else still runs"). They passed in the engine and failed in rekstrarkerfid the day it really switched `signup` off in `config/client.json`. rk had already forked one of them to switch party back on, so each new module would have meant another fork. The fix is to build the default in the test itself: every catalogued module on by env (`envNameFor(['modules', id, 'enabled'])`), and `createGate({ config: clientConfig.defaults() })`. The same day the MCP realm literal and a `party` pick in the R5b tests had the same flaw. Rule: an engine test pins the ENGINE, never the product it happens to run in; `engine.json.role` gates are for pins about THIS repo.

### 2026-09-24 — String.replace with a string replacement rewrote deploy.yml

_(factory)_ Harvest chunk F inserted a health-check step into `deploy.yml` with `s.replace(anchor, newText)`. `newText` held the shell pattern `grep -Eq '^[0-9]+$'`, and in a replacement STRING `$'` means "everything after the match", so the rest of the file (the alert job) was pasted into the middle of that line and the workflow stopped parsing. Nothing reports a broken dispatch-only workflow until someone tries to deploy; rekstrarkerfid's sync noticed it. Fixing it, the same slip hit `docs/ARCHITECTURE.md` (+125 lines) within the hour. Rule: never pass a string replacement that contains `$` — pass a function (`s.replace(a, () => text)`) or splice by index. `tests/unit/workflowsParse.test.js` now parses every workflow.

### 2026-09-25 — something emptied an engine-sync worktree mid-run

_(factory)_ During `engine-sync.js --continue` in `rekstrarkerfid/.wt/engine-sync-2026-09-25`, 159 tracked files (`docs/`, `features/`, `e2e/`, `.github/`, most of `tests/`) and part of `node_modules` vanished from the working tree; the files in use survived. The sync itself only runs `npm ci` + lint + i18n + `test:ci`, none of which touch tracked files, so the likely cause is an outside delete of the untracked `.wt/` (rk's `.gitignore` does not list it; another session had a worktree on the same commit) stopped partway by Windows file locks. It was NOT proven. The index and `MERGE_HEAD` were intact: `git checkout-index -a -f` restored the staged merge exactly, and the rerun (watched by a Monitor for missing dirs) was clean. **Rule:** if a sync's verification fails with ENOENT inside the worktree, check `git status` for mass ` D` before touching code; restore from the index, never from origin. Proposed: add `.wt/` to every downstream's `.gitignore` so no session reads it as stray untracked files.

### 2026-09-26 — the Edit tool rewrote os.js line endings, including 88 lone CRs

_(project)_ `server/config/product-migrations/os.js` is committed `-text` with mixed terminators: 264 CRLF and 88 lone `\r`. One Edit-tool insertion for os_002 normalised every terminator to CRLF, and `git diff --numstat` showed 316/264 instead of +52. The lone CRs sit near the applied os_001 text, so a silent rewrite risks the seed-equals-migration pin (`salesGuidesD001.test.js`) and the byte history of an applied entry. The fix was to rebuild the file from the HEAD blob and splice the new blocks in with a node script (function replacements). **Rule:** before editing a `-text` file (`git ls-files --eol`), count its terminators, and edit it with a byte-preserving script, never the Edit tool. Check `--numstat` after any edit to os.js.

### 2026-09-26 — one hard-coded prefix made every repo's test databases the engine's

_(factory)_ `tests/workerDb.js` and `e2e/lib/dbUrl.js` built every test-database name from the literal `orangesmiley`, and every downstream (rekstrarkerfid, hallismiley, LedgerLink) carries those files byte-identical. On the one shared `:5432` server they all derived the SAME names as the engine — `orangesmiley_master_w1_test`, `orangesmiley_engine_sync_<date>_…` — so one repo's teardown or `test:db:clean` could drop another repo's live run, and nobody could tell whose a leftover was: 746 of them had piled up, with 8-minute checkpoints behind them. Per-branch scoping (2026-09-02) fixed collisions between worktrees of ONE repo and hid this one. Fix: the prefix is `engine.json` `product`, every database is labelled with its repo/branch/pid at creation, and a sweep on every run drops the dead ones ([test-db-hygiene](docs/history.d/2026-09-26-feat-test-db-hygiene.md#test-db-hygiene-2026-09-26)). Rule: an engine file that names a shared resource must take the name from the product seam, never a literal — a downstream inherits the literal.

### 2026-09-26 — a TEST_DATABASE_URL in .env is silently ignored by Jest

_(factory)_ LedgerLink's `.env` pinned `TEST_DATABASE_URL`, and its runs used the derived per-branch name anyway: the Jest MAIN process never loads `.env` (on purpose — its env is inherited by every worker, and `tests/env.js` controls what the app sees), and `tests/workerDb.js` parses only named keys out of the file. Nothing said so. Now globalSetup prints a note when `.env` has a `TEST_DATABASE_URL` the run is not using, and the server knob that IS read from `.env` is `TEST_PG_URL` ([test-db-hygiene](docs/history.d/2026-09-26-feat-test-db-hygiene.md#test-db-hygiene-2026-09-26)). Rule: a setting read from `.env` by test tooling is parsed by name in `workerDb.js`; anything else there does nothing for Jest.

### 2026-09-26 — TRUNCATE between tests cost 7× the suite, and fsync=off did not help

_(factory)_ The throwaway test cluster was meant to make the cleanup churn free (fsync, full_page_writes, synchronous_commit all off). It did not: a full run still took 389 s against 49 s with DELETE, and an fsync=on copy showed the same ratio (347 s against 49 s). Every `TRUNCATE … RESTART IDENTITY CASCADE` gives each table in the users FK closure new files, and each statement took 0.4–0.9 s on Windows. The old files are unlinked at the next checkpoint, and every `DROP DATABASE` forces one: 121 s for one checkpoint with fsync off, almost all of it unlinks, not sync. So measure before blaming fsync, and prefer DELETE for small per-test cleanups ([test-db-hygiene](docs/history.d/2026-09-26-feat-test-db-hygiene.md#test-db-hygiene-2026-09-26)).
