...
Two "busy, send it again" shapes extend it with `reason` + `retryable`:

| Status | Body | When |
|--------|------|------|
| `409` | `{ "error": …, "code": 409, "reason": "BUSY", "retryable": true }` | Postgres picked the request as a deadlock victim (40P01); nothing was changed |
| `429` + `Retry-After: <seconds>` | `{ "error": …, "code": 429, "reason": "AI_BUSY", "retryable": true }` | a request-path paid AI call found every `aiGate` slot taken (`AI_MAX_CONCURRENT`, default 4; `server/services/aiGate.js`, harvest2 2026-09-26). No engine route raises it yet — the translator, today's only Claude caller, queues for a slot instead and never fails a save — so it is the contract for the next request-path AI endpoint |
