...
| Jest | `tests/integration/auth.test.js`, `auth.google.test.js`, `auth.facebook.test.js`, `auth.socialKillSwitch.test.js`, `users.test.js`, `adminRoles.test.js`, `adminTotp.test.js`, `adminTotpEnforcement.test.js`, `mfaReminder.test.js`, `security.test.js`, `adminOuterGuard.test.js`, `adminNameOnlyLogin.test.js`, `authLoginMetrics.test.js`, `signupEmailNonBlocking.test.js`; `tests/unit/adminUsersView.client.test.js`, `totp.test.js`, `totpFailure.client.test.js`, `mfaPolicy.test.js`, `mfaProtected.test.js`, `mfaProtectedClient.test.js`, `oauthHelpers.test.js`, `csrf.test.js`, `safeReturnTo.client.test.js`, `rateLimit.test.js`, `rateLimitDecide.test.js`, `rateLimitGuard.client.test.js`, `nameOnlyHelpers.test.js`, `generatePassword.test.js` |
...
- **Every role and account change on the admin surface writes a staff-audit
  row**, whichever screen made it: the Users-page role dropdown writes
  ...
  2026-09-26.
- **Signup never waits on email**: the verification send is detached
...
- **`auth_login_attempts_total{result}`** is incremented on every sign-in
...
- **A cancelled `confirm()` says so** on the Users page (and the party
...
- **The Users page's Party column follows the party module**
...
**Rules that must hold**
- **The nav never adds a listener per render**: `_renderAuth` runs again on
...
**Rules that must hold**
- **Every paid Claude call takes an `aiGate` slot** (`AI_MAX_CONCURRENT`,
  ...
  10 s grace.
- **The translator salvages JSON wrapped in prose** (`parseJsonArray`, from
...
- `staffAudit.ACTIONS` is the closed vocabulary: an action written anywhere
...
**Rules that must hold**
- **A metric that is defined is incremented**: `auth_login_attempts_total`
...
| Jest | `tests/unit/schema-integrity.test.js`, `database.test.js`, `workerDb.test.js`, `featureGate.test.js`, `errorHandlerDeadlock.test.js`, `errorHandlerAiBusy.test.js`, `migrationIdempotent.test.js`, `ciSkippedShim.test.js`, `workflowsParse.test.js`; `tests/integration/migrateRunner.test.js` |
...
**Rules that must hold**
- **A typed "come back later" goes through the central error middleware**:
