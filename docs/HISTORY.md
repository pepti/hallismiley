...
| 2026-09-26 | [The demo instance: sample data, reset every night](#demo-instance-2026-09-26) | `DEMO_INSTANCE=true` only on a demo environment (`APP_ENV=demo`, `DEMO_DATABASE_NAME` pinned; checked at boot, a misplaced flag exits); email, payments, MCP and IndexNow off whatever the env holds; robots `Disallow: /` + `X-Robots-Tag`; a banner; the reset: JSON snapshot of the kept accounts → drop `public` → whole chain → one-statement restore per table in a transaction → the product's `server/demo/seed.js` → uploads emptied → exit; an interrupted reset recovers at boot; nightly at `DEMO_RESET_HOUR_UTC` (default 02) and from `/admin/general` (checked before the 202, 15-min cooldown); no migration |
