...
- **A worked example for sellers**, right under it: a customer on Rekstur
  ...
  work starts.
- **Migration `os_004_sales_guides_queue_spread`**: one exact old → new
  passage through os_001's `guideEdit` helper, so the same guard as os_003
...
- **Tests**: the new `salesGuidesQueueSpread.test.js` checks that the old
  passage occurs once in the os_003 text and the new one once in the seed, that
  no guide keeps the "next month" promise, that os_004 turns the os_003 text
...
- Not changed: the sentence above the list, "Stórt verk (20 einingar) er stærra
  en mánaðarskammtur nokkurs þreps, svo það sem umfram er greiðist á
  einingaverði", which names only the pay-now path; the example below it now
  shows both. Whether unused units carry over is still "DRÖG — Halli
  staðfestir".
