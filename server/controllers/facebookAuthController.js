...
function clearOAuthCookies(res) {
  ...
  res.clearCookie('facebook_oauth_return_to');
}
...
async function start(req, res, next) {
  try {
    ...
    if (isSafeReturnTo(req.query.returnTo)) {
      setOAuthCookie(res, 'facebook_oauth_return_to', req.query.returnTo);
    }
...
async function callback(req, res, next) {
  try {
    ...
    const rawReturnTo     = req.cookies?.facebook_oauth_return_to;
    ...
    // above (state, return_to) that were appended earlier.
