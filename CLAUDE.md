...
Full rules: `.claude/rules/stack-invariants.md` (auto-loaded). Two footguns worth repeating here: Express 5 catch-alls are `app.get('/{*splat}', …)` — keep the braces, `'/*splat'` stops matching `/`; and the Node major is pinned in TWO places (24 LTS today — the `Dockerfile` digest and ci.yml `node-version` move TOGETHER, and dependabot must not major-bump the base image on its own; `promote.yml` still says 20, a known drift).
...
- **Docs sync 2026-09-11** (branch `docs/drift-sync-2026-09-11`): every tracked markdown file was checked against the code — three read-only audits by doc family, every claim written re-verified against a source line. Rewritten for this repo because they were still the base's: `README.md`, `docs/DEPLOYMENT.md`, `CHANGELOG.md` (now keyed `## [0.1.0]` so `scripts/build-manifest.js` finds a section — the inherited `[1.0.0]` would have published an empty changelog). Deleted: `docs/SHOP_REDESIGN.md` (portfolio storefront plan; steps 1–2 shipped as 045, the rest superseded — the two code comments citing it were repointed). Frozen with banners: `PRE_LAUNCH_AUDIT.md`, `SECURITY_AUDIT_2026-04-16.md` (15/16 findings closed here, 3.13 open), `SELF-UPDATE-PLAN.md`. Fixed in place: RUNBOOK (`/health` has no database key, deploy is dispatch-only, placeholders instead of the base's Azure names), TESTING (4 workers, dated counts, what CI runs), SECURE_SDLC v1.2, API (CSRF, 2FA branch, real limits, router inventory), BOOKKEEPING-SYSTEM, mcp (2 tools, mount order — also the `mcpRoutes.js` header comment), SLO, SALES-STAFF, SELF-UPDATE, ENHANCEMENTS, PLAN, this file. **Code defects found, flagged, NOT fixed there**: `promote.yml` Node 20; `setup.ps1` generates unused RSA keys and ends by recommending `/strip-base`; `mcpAuth.js` realm and the MCP panel's connector name still say `icelandicstore`; `/documents/:id` has `docLimiter` after the view check; MCP arguments pass through `sanitizeBody` and count against the global IP limit; `railway.toml` and the `nodemailer` dependency are dead; `.env.example` carried dead SMTP settings (cleaned).