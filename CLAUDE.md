...
- Express 5.x, CommonJS server. PostgreSQL via `pg` (dev DB `orangesmiley`; test DBs derived **per product + branch** as `<engine.json product>_<branch-slug>_test` (`os_…` here) — one template + one per Jest worker, `…_w<N>_test`, `tests/workerDb.js` — created on the throwaway test server `TEST_PG_URL` (`:5433`, never a server with real data) when it is set; user postgres/postgres).
...
- **After merging a chunk, remove what it left** (2026-09-26): unlink the worktree's `node_modules` junction with `cmd //c rmdir` FIRST (never `rm -rf` a junction — it empties the source), then `git worktree remove`, `git branch -d`, and `npm run test:db:clean -- --gone --yes` for its test databases (`docs/TESTING.md`).
