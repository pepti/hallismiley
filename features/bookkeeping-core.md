...
since: 2026-08-09
origin: null
history: [accounts-commission, review-099, migrations-100-102, ui-kit, harvest2-lane1a-2026-09-26]
---
...
- `seed:books` runs only where `targetGuard` allows (a local `_test` DB, or the local dev DB with `--allow-dev-db`; never `*_books`), and its `--wipe` deletes only the rows the seed created, by id, in one transaction — it refuses (`UNSEEDED_BOOKS`, nothing deleted) while the books hold any other row ([harvest2-lane1a](../docs/history.d/2026-09-26-harvest2-lane1a-security.md#harvest2-lane1a-2026-09-26); ported from icelandicstore #427).
