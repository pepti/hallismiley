...
describe('GET /auth/facebook/callback', () => {
  ...
  test('with public signup switched off, a NEW Facebook account is refused', async () => {
    const modules = require('../../server/config/modules');
    await modules.setModuleSwitch('signup', false);
    try {
      const res = await request(app).get('/auth/facebook/callback?code=abc&state=test-state-xyz').set('Cookie', cookieHeader);
      expect(res.status).toBe(302);
      expect(res.headers.location).toMatch(/error=signup_closed$/);
      const { rows } = await db.query('SELECT 1 FROM users WHERE facebook_id IS NOT NULL');
