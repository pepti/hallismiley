...
//                     (Halli asked for that placement, 2026-08-09)
//   • AdminBackgroundView (/admin/background) — the standalone admin page the
//                     base shipped, kept working so no capability is lost
//
// Emits nothing; each save PATCHes the server and repaints locally. The public
// home page reads the same row on its next load
// (GET /api/v1/content/landing_background), so no cross-component wiring.
...
export class LandingBackgroundAdmin {
  /**
   * @param {object}  [opts]
   ...
   *   admin page supplies its own heading, so it passes false.
   */
  ...
  render() {
    ...
    this._el.className = 'bg-admin';
    this._el.innerHTML = this._section
      ? `<section class="profile-section">
           <h2 class="profile-section__title">${escHtml(t('adminBg.title'))}</h2>
           <p class="profile-section__hint">${escHtml(t('adminBg.subtitle'))}</p>
           <div id="bg-body"><div class="admin-loading">${escHtml(t('form.loading'))}</div></div>
         </section>`
      : `<div id="bg-body"><div class="admin-loading">${escHtml(t('form.loading'))}</div></div>`;
    this._load();
    return this._el;
    ...
    this._el.querySelector('#bg-body').innerHTML = `
      ...
      <div class="bg-lib">
        <div class="bg-lib__head">
          <h3 class="bg-lib__title">${t('adminBg.library')}</h3>
