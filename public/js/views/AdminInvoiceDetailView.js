...
// did what. Three actions: record a payment, record a refund, issue a credit note.
...
//
// Refund and credit note are separate on purpose. A refund moves cash; a credit
// note reverses the sale and its VAT. A normal return is both, a chargeback is a
...
export class AdminInvoiceDetailView {
  constructor(id) {
    ...
    this._refundKey = newIdempotencyKey();
    this._busy = false;
    // Discards a response that lands after the view has been navigated away from,
    // matching the sibling books views.
    this._generation = 0;
  }
        ...
        <table class="books-table">
          <tbody>
            ...
            ${inv.amount_refunded > 0
              ? `<tr><td>${escHtml(t('adminBooks.detail.refunded'))}</td>
                   <td class="num">+${escHtml(isk(inv.amount_refunded))}</td></tr>` : ''}
            <tr class="books-totals__grand">
  ...
  _renderActions() {
    ...
    // You can only hand back money you actually received.
    const refundable = Number(inv.amount_paid || 0) - Number(inv.amount_refunded || 0);
    return `
      <section class="books-section">
        ...
        <div class="books-columns">
          ...
          <form id="refund-form" class="books-form" ${refundable <= 0 ? 'hidden' : ''}>
            <h3>${escHtml(t('adminBooks.detail.recordRefund'))}</h3>
            <p class="admin-shop__hint">${escHtml(t('adminBooks.detail.refundHint'))}</p>
            <label>${escHtml(t('adminBooks.detail.amount'))}
              <input type="number" name="amount" min="1" step="1" required
                     max="${escHtml(String(refundable))}"
                     value="${escHtml(String(refundable))}" />
            </label>
            <label>${escHtml(t('adminBooks.detail.method'))}
              <select name="method">
                ${METHODS.map(m => `<option value="${escHtml(m)}">${
            ...
            <button type="submit" class="btn btn--ghost">
              ${escHtml(t('adminBooks.detail.recordRefund'))}
            </button>
          </form>
  ...
  _wireActions() {
    ...
    const refundForm = this._el.querySelector('#refund-form');
    if (refundForm) {
      refundForm.addEventListener('submit', async (e) => {
        e.preventDefault();
        if (this._busy) return;
        const fd = new FormData(refundForm);
        if (!window.confirm(t('adminBooks.detail.confirmRefund'))) return;
        await this._submit(refundForm, () => recordRefund(this._id, {
          amount: Number(fd.get('amount')),
          method: fd.get('method'),
          received_at: fd.get('received_at') || undefined,
          reference: fd.get('reference') || '',
          idempotency_key: this._refundKey,
        }), t('adminBooks.detail.refundRecorded'));
      });
    }
  ...
  destroy() {
    // Bumping the generation makes any in-flight _load() discard its response
    // instead of painting into a node that is no longer in the document.
    this._generation += 1;
  }
}
